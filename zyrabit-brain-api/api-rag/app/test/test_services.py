...
def test_get_SLM_router_decision_calls_SLM_and_parses_response(
        mock_httpx_post):
    """
    Prueba que la función get_SLM_router_decision:
    1. Llame a httpx.post con la URL y el meta-prompt correctos.
    2. Parseé la respuesta JSON del SLM.
    ...
    # El primer argumento de post()
    called_url = mock_httpx_post.call_args[0][0]
    assert called_url == f"{MOCK_SLM_URL}/api/generate"
...
@patch.dict('os.environ', {'SLM_URL': MOCK_SLM_URL})
@patch('httpx.post')
def test_call_direct_SLM_sends_clean_query_and_returns_response(
        mock_httpx_post):
    """
    Prueba que la función call_direct_SLM (la ruta "fácil"):
    1. Llame a httpx.post con la URL y el payload correctos.
    2. El prompt enviado sea EXACTAMENTE la query del usuario.
    3. Devuelva la respuesta limpia del SLM.
    """
    # 1. GIVEN (Dado...)
    ...
    response_text = services.call_direct_SLM(query)
    ...
    # ¡Crítico! El prompt es la query directa
    assert call_payload['prompt'] == query
...
def test_execute_rag_pipeline_retrieves_augments_and_generates(
        mock_httpx_post, mock_chroma_client):
    """
    ...
    mock_SLM_response = MagicMock()
    mock_SLM_response.status_code = 200
    mock_SLM_response.json.return_value = {
        "response": "Robert C. Martin opina que el framework es un detalle.",
        "done": True
    }
    mock_httpx_post.return_value = mock_SLM_response
    ...
    mock_collection.query.assert_called_with(
        query_texts=[query], n_results=5)  # Asumimos n_results=5
...
@patch.dict('os.environ', {'SLM_URL': MOCK_SLM_URL})
@patch('httpx.post')
def test_get_SLM_router_decision_network_error(mock_httpx_post):
    """
    Prueba que get_SLM_router_decision retorne un fallback seguro cuando hay error de red.
    """
    # GIVEN
...
@patch.dict('os.environ', {'SLM_URL': MOCK_SLM_URL})
@patch('httpx.post')
def test_get_SLM_router_decision_invalid_response(mock_httpx_post):
    """
    Prueba que maneje respuestas inválidas del SLM (decisión no reconocida).
    """
    # GIVEN
    mock_api_response = MagicMock()
    mock_api_response.status_code = 200
...
@patch.dict('os.environ', {'SLM_URL': MOCK_SLM_URL})
@patch('httpx.post')
def test_call_direct_SLM_timeout(mock_httpx_post):
    """
    Prueba que call_direct_SLM maneje timeouts correctamente.
    """
    # GIVEN
...
@patch('chromadb.HttpClient')
@patch('httpx.post')
def test_execute_rag_pipeline_empty_results(
        mock_httpx_post, mock_chroma_client):
    """
