...
def test_router_sends_tech_queries_to_RAG(mock_router_decision, mock_rag_pipeline):
    """
...
@patch('app.services.execute_rag_pipeline')
@patch('app.services.get_llm_router_decision')
def test_router_sends_general_queries_to_DIRECT_LLM(mock_router_decision, mock_rag_pipeline, mock_direct_llm):
    """
    Prueba que una pregunta sobre el clima sea enrutada a 'direct_llm_answer'.
    """
    # 1. GIVEN
    mock_router_decision.return_value = "direct_llm_answer"
    mock_direct_llm.return_value = "[Respuesta LLM Directa para: ¿Cómo estará el clima mañana?]"
    ...
    assert response.status_code == 200
    assert "Respuesta LLM Directa para" in response.json()['response']

# --- PRUEBA 3: El Router sabe cuándo RECHAZAR (El Guardrail) ---

@patch('app.services.execute_rag_pipeline')
@patch('app.services.get_llm_router_decision')
def test_router_REJECTS_out_of_scope_queries(mock_router_decision, mock_rag_pipeline):
    """
