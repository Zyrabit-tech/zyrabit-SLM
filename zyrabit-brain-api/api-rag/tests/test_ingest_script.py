...
def test_check_ollama_model_unavailable(mock_embeddings):
    ...
    # GIVEN
    mock_embeddings_instance = mock_embeddings.return_value
    mock_embeddings_instance.embed_query.side_effect = Exception("Model not found")
...
def test_load_documents_success(mock_pdf_loader, mock_listdir, mock_exists):
    ...
    mock_listdir.return_value = ["document1.pdf", "document2.pdf", "readme.txt"]
...
def test_split_documents_correct_chunks():
    ...
    for chunk in chunks:
        assert len(chunk.page_content) <= 1200  # Con overlap podría ser un poco más
...
def test_populate_vector_db_batching(mock_embeddings, mock_chroma_client):
    ...
    mock_embeddings_instance.embed_documents.return_value = [[0.1, 0.2] for _ in range(250)]
