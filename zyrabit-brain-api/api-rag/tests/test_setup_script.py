...
def test_setup_ollama_script_contains_models():
    ...
    script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../setup_ollama.sh"))
    
    if not os.path.exists(script_path):
        # Intentar ruta alternativa si estamos en la raíz del repo
        script_path = os.path.abspath("setup_ollama.sh")
    
    assert os.path.exists(script_path), f"No se encontró setup_ollama.sh en {script_path}"
