...
def test_end_to_end_rag_flow(mock_execute, client):
...
@patch('app.domain.use_cases.chat_use_case.ChatUseCase.execute')
def test_end_to_end_direct_slm_flow(mock_execute, client):
    mock_execute.return_value = {
...
@patch('app.domain.use_cases.chat_use_case.ChatUseCase.execute')
def test_end_to_end_reject_query_flow(mock_execute, client):
    mock_execute.return_value = {
...
@patch('app.domain.use_cases.chat_use_case.ChatUseCase.execute')
def test_multiple_queries_flow(mock_execute, client):
...
def test_health_check_before_operations(mock_get_db, mock_get_inf, client):
    response = client.get("/v1/health")
    assert response.status_code == 200

@patch('app.domain.use_cases.chat_use_case.ChatUseCase.execute')
def test_error_recovery_flow(mock_execute, client):
    mock_execute.return_value = {
